...
def build_case_embeddings(input_path: Path, output_path: Path, index_path: Path, model=None, text_mode: str = "baseline") -> tuple[int, int]:
    ...
    embeddings = model.encode([case_embedding_text(record, mode=text_mode) for record in records], normalize_embeddings=True, convert_to_numpy=True, show_progress_bar=True)
...
def main() -> int:
    ...
    parser.add_argument("--text-mode", choices=("baseline", "optimized"), default="baseline")
    args = parser.parse_args()
    count, dimension = build_case_embeddings(Path(args.input), Path(args.output), Path(args.index), text_mode=args.text_mode)
